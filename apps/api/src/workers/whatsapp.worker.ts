...
const worker = new Worker(
  ...
  async (job) => {
    ...
    console.log('💬 Processing WhatsApp recovery for order:', orderId);
    ...
    if (order.status === 'RECOVERED') {
      console.log(`⏭ Order ${orderId} already recovered, skipping WhatsApp`);
      return;
    }

    if (order.whatsappSentAt) {
      console.log(`⏭ WhatsApp already sent for ${orderId}, skipping`);
      return;
    }
    ...
    const defaultTemplate = `Hi {{customerName}}, you left ${{cartValue}} in your cart at {{storeName}}! Complete your order here: {{recoveryLink}}`;
    const rawTemplate = campaign?.whatsappTemplate || defaultTemplate;
    ...
    const fromNum = order.store.owner?.twilioWhatsappNum || process.env.TWILIO_WHATSAPP_NUM;
    if (!fromNum) throw new Error('Twilio WhatsApp number not configured');
...
worker.on('completed', (job) => console.log('✅ WhatsApp job done:', job?.id));
worker.on('failed',    (job, err) => console.error('❌ WhatsApp job failed:', job?.id, err.message));
